...
Chain Monitor uses local LLM inference (Ollama) for two capabilities:

### 1. Twitter Semantic Enrichment
Every tweet, retweet, and quote-tweet is enriched with semantic understanding:
- **Full content storage**: All tweet text, metrics, media URLs, and thread context
...
### 2. LLM-Powered Daily Digest
Instead of blunt signal enumeration, the digest can be generated by LLM:
- **Narrative synthesis**: Unifying themes across disparate events
- **Connection drawing**: e.g. "Three L2s announced mainnet — broader 'L2 maturity' narrative"
- **Prose format**: Faster to parse than bullet lists for busy analysts
- **Template fallback**: If LLM unavailable, proven template-based formatter runs instead
- **Gated by env**: Set `LLM_DIGEST_ENABLED=true` in `.env` to enable

### LLM Configuration
All LLM settings are configurable via `.env` (no code changes):
