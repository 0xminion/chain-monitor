#!/usr/bin/env python3
"""End-to-end pipeline with Twitter stubbed to avoid timeout.

On this Steam Deck, 15 parallel Playwright workers scraping 138 handles
exceeds practical runtime limits. We run non-Twitter collectors (which
complete in ~90s) and stub Twitter to return empty, simulating a
quiet weekend with no tweets in the 24h lookback window.

All other collectors run live — no caches."""
...
async def main():
    ...
    # Monkeypatch TwitterCollector.collect to return empty instantly
    original_collect = TwitterCollector.collect
    TwitterCollector.collect = lambda self: []
    ...
    try:
        ...
        TwitterCollector.collect = original_collect
    ...
    report = {
        ...
        "twitter_stubbed": True,
        "note": "Twitter collector stubbed to avoid Steam Deck timeout. All other collectors live.",
    }
